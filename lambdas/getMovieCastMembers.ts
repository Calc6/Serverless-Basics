...
export const handler: APIGatewayProxyHandlerV2 = async (event, context) => {
  try {
    ...
    if (queryParams.facts === "true") {
      const movieClient = new DynamoDBClient({ region: process.env.REGION });
      const movieResponse = await movieClient.send(
        new QueryCommand({
          TableName: "Movies",
      ...
      const movieDetails = movieResponse.Items ? movieResponse.Items[0] : null;
